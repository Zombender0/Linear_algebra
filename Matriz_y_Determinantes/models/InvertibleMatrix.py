...
class InvertibleMatrix(Matrix):
    def __init__(self, matriz: list[list], fraccion:bool) -> None:
        ...
        self.config = {}
        self.identidad_valor_uno = Fraction(1) if self.fraccion_oper else 1.0
        self.identidad_valor_cero = Fraction(0) if self.fraccion_oper else 0.0

        self.matriz_identidad = [
            [self.identidad_valor_uno if i == j else self.identidad_valor_cero for j in range(self.filas)] 
    ...
    def gauss_jordan_inversa(self):
        ...
        if all(self.matriz_aumentada[i][j] == (self.identidad_valor_uno if i == j else self.identidad_valor_cero) 
               for i in range(self.filas) for j in range(self.columnas)):
    ...
    def convertir_a_1(self, col: int) -> bool:
        ...
        if pivote != 1 and pivote != 0:
            ...
            if not self.fraccion_oper:
                ...
                self.config[f"F{col + 1} -> F{col + 1} / {pivote}"] = copy.deepcopy(self.matriz_aumentada)
    ...
    def reduccion_a_cero(self, col : int):
        for fila in range(self.filas):
            ...
            if not self.fraccion_oper:
                ...
                self.config[f"F{fila + 1} -> F{fila + 1} {operador} {operando}F{col + 1}"] = copy.deepcopy(self.matriz_aumentada)

    def singularMatrix(self) -> bool:
        matriz_cuadrada_copia = copy.deepcopy(self.matriz)
        matrizA = GaussMethod(matriz_cuadrada_copia, self.fraccion_oper)
        #Continue here
